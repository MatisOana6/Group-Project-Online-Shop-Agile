...
import com.backend.exception.ExceptionBody;
    ...
    protected void doFilterInternal(
        ...
        try {
            Claims claims = Jwts.parser()
                    .setSigningKey(JwtService.getSigningKey(JwtService.getSecretKey()))
                    .parseClaimsJws(jwtToken)
                    .getBody();

            String username = claims.getSubject();
            Collection<SimpleGrantedAuthority> authorities = ((List<String>) claims.get("role")).stream()
                    .map(role -> new SimpleGrantedAuthority("ROLE_" + role)) // <- AICI e modificarea importantă
                    .toList();
